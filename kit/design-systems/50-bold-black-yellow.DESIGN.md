# DESIGN.md — ボールド・ブラック&イエロー（Bold Black & Yellow）
...
| `--accent` | 差し色（番号・枠線・バッジ・矢印） | `#FACC15` |
| `--accent-deep` | 差し色の濃い版（バッジ影など） | `#CA9A04` |
| `--bg` / `--paper` | 背景 / カード白面 | `#FFFFFF` / `#FFFFFF` |
| `--ink` / `--ink-mid` | 本文 / 補助テキスト | `#1A1A1A` / `#444444` |
| `--ink-sub` | 極淡（**罫線など非テキスト専用**） | `#8A8A8A` |
| `--on-accent` | accent 地の上に乗せる文字色 | `#1A1A1A` |
...
:root{
  ...
  --accent:#FACC15; --accent-deep:#CA9A04; --accent-soft:rgba(250,204,21,0.16);
  --bg:#FFFFFF; --paper:#FFFFFF; --ink:#1A1A1A; --ink-mid:#444444; --ink-sub:#8A8A8A;
  --line:rgba(26,26,26,0.12); --on-accent:#1A1A1A; --canvas:#E6E6E6;
  --head:'Noto Sans JP';
}
```
...
- バッジ・ボタン・番号は **accent 地 ＋ `--on-accent` の文字**（このテーマでは `#1A1A1A`）。
...
## 1行で使う
```
このフォルダの make-slides スキルで、デザインは design-systems/50-bold-black-yellow.DESIGN.md を使ってスライドを作って。テーマは「◯◯」。
```
